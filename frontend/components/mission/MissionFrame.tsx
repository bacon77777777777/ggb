...
const ACHIEVEMENT_TITLE: Record<string, { name: string; color: string }> = {
  'draw_count:500':        { name: '抽獎狂熱者', color: 'purple' },
  ...
  return (
    ...
    >
      ...
      <RulesModal 
        ...
        title="積分規則"
        /*
         * 內容依實際規則重寫（老闆 2026-08-20）。原本那六條有兩處與現況不符：
         * 寫「連續天數越多獎勵越豐富」（實際是固定 20，第七天 100），
         * 以及「可兌換商城商品」（積分從來不能買商城商品，只能在抽獎時折抵）。
         * 玩家照著做卻拿不到，比沒寫還糟。
         */
        rules={[
          ...
          "促銷活動的商品只能用代幣抽，不能用積分，也不能再折優惠券。",
          "積分是平台送的回饋，不能換現金、不能轉給別人，退款時也不會計算在內。",
          "用外掛或不正常方式刷積分，我們會收回積分並停用帳號。"
        ]}
      />
