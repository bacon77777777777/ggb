...
export default function SellFormContent({ editId: propEditId, onDone }: { editId?: number | null; onDone?: () => void }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { user, isLoading } = useAuth();
  const { showToast } = useToast();

  // 彈層情境沒有網址參數，改由外層帶入；路由情境仍讀 ?edit=
  const editId =
    propEditId ??
    (() => {
