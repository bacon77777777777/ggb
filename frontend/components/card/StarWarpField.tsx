 ...
 *
 * ⚠️ 這支跟商品頁卡包輪播（`PackShowcase3D`）沒有關係。那邊以前有一組
 * `.ggb-meteor` 的 CSS 流星，老闆 2026-09-01 已指定移除；**不要因為那邊空了
 * 就把這支搬過去頂替** —— 兩者的目的不同（那邊要的是靜謐的遠景，這裡要的是
 * 撕開瞬間的速度感），而且這裡幾百顆各自有深度與透視，只能走 canvas。
 *
