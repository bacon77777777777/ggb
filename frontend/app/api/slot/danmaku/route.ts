...
export async function GET() {
  try {
    ...
    const [realRes, botRes] = await Promise.all([
      supabase
        .from('slot_spin_logs')
        .select('id, user_id, prize_name, prize_value, machine_id, users!inner(name, avatar_url, is_bot)')
      ...
      supabase
        ...
        .select('id, user_id, prize_name, prize_value, users(name, avatar_url)')
    ...
    const shape = (r: any, idPrefix: string) => ({
      ...
      name: displayName(r.users?.name ?? null),
