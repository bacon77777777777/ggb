...
function css(vars: { bg: string; accent: string }) {
  return `
    .lpv { --bg:${vars.bg}; --accent:${vars.accent}; --accent20:${vars.accent}33; --accent50:${vars.accent}80;
      background:var(--bg); color:#fff; min-height:100svh; overflow-x:hidden;
      font-family:'Noto Sans JP',system-ui,sans-serif; }
    .lpv-back { position:fixed; top:14px; left:14px; z-index:60; display:flex; align-items:center;
      gap:6px; padding:8px 14px; border-radius:999px; background:rgba(0,0,0,.5);
      backdrop-filter:blur(8px); color:rgba(255,255,255,.85); font-size:13px; font-weight:700;
      border:1px solid rgba(255,255,255,.15); cursor:pointer; text-decoration:none; transition:opacity .15s; }
    .lpv-back:hover { opacity:.8; }
    /* Hero */
    .lpv-hero { position:relative; min-height:100svh; display:flex; flex-direction:column;
      align-items:center; justify-content:center; text-align:center; padding:80px 24px 40px; overflow:hidden; }
    ...
    .lpv-eyebrow { position:relative; z-index:1; font-size:11px; letter-spacing:7px;
      color:var(--accent); opacity:.8; font-weight:800; margin-bottom:14px; text-transform:uppercase; }
    .lpv-title { position:relative; z-index:1; font-weight:900; line-height:.95;
      letter-spacing:2px; font-size:clamp(44px,13vw,88px);
    ...
    .lpv-cta-btn { display:inline-flex; align-items:center; gap:8px; margin-top:24px;
      ...
      box-shadow:0 10px 34px var(--accent50); position:relative; z-index:1; text-decoration:none; transition:transform .15s; cursor:pointer; }
    ...
    .lpv-footer-btn { display:block; text-align:center; border-radius:999px; padding:17px;
      font-weight:900; font-size:17px; color:#1a1208; text-decoration:none;
...
// ─── Section renderers ────────────────────────────────────────────────────────
function HeroSection({ c, accent }: { c: Record<string, unknown>; accent: string }) {
  ...
  return (
    <section className="lpv-hero">
      ...
      <h1 className="lpv-title">{c.title as string}</h1>
      {gems.length > 0 && (
        <div className="lpv-gems">{gems.map((g, i) => <i key={i} style={{ background: g.color, color: g.color }} />)}</div>
      )}
      ...
      {c.cta_url && (
        <Link href={c.cta_url as string} className="lpv-cta-btn">▶ {(c.cta_text as string) || '立即參加'}</Link>
      )}
...
export default function LpPage() {
  ...
  return (
    <div className="lpv" style={{ position: 'fixed', inset: 0, zIndex: 50, overflowY: 'auto', background: event.bg_color }}>
      ...
      {sections.map(sec => {
        switch (sec.type) {
          case 'hero': return <HeroSection key={sec.id} c={sec.content} accent={event.accent_color} />
