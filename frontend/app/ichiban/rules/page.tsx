...
const steps = [
  {
    ...
    items: [
      {
        label: '立即抽取',
        desc: '點擊「立即抽取」，使用 G幣 確認購買並抽取賞項。每次抽取結果即時產生，無法取消或重抽。',
      },
      {
        label: '賞項說明',
        desc: '一番賞各賞別數量固定，依商品設定顯示剩餘數量。抽取後賞項從剩餘中隨機決定，無法指定特定賞別。',
      },
    ],
  },
  {
    ...
    items: [
      {
        label: '查看結果',
        desc: '抽取結果即時顯示，賞品自動存入帳號「我的倉庫」。可前往「個人中心 → 抽獎紀錄」查看歷史記錄。',
      },
    ],
  },
  {
    ...
    items: [
      {
        label: '前往我的倉庫',
        desc: '進入「個人中心 → 我的倉庫」選取賞品後申請配送。倉庫提供 30 天免費寄存，逾期系統將自動回收品項並返還相應代幣，請務必在期限內申請配送。',
      },
    ],
  },
];

export default function IchibanRulesPage() {
  ...
  const rules = [
    ...
    {
      label: '倉庫寄存',
      desc: '賞品存入我的倉庫後提供 30 天免費寄存。第 31 天起，系統將自動回收未申請配送的品項並返還相應代幣至帳戶，請務必在期限內申請配送。',
    },
    {
      label: '免運條件',
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 pb-16 safe-header-offset">
      <SimplePageHeader title="一番賞規則" onBack={() => router.back()} maxWidth="max-w-[960px]" />
      ...
      <div className="max-w-[560px] md:max-w-[960px] mx-auto px-4 py-6 space-y-4 md:space-y-8">
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-100 dark:border-neutral-800 px-5 py-5">
          ...
          <p className="text-[13px] text-neutral-500 dark:text-neutral-400 leading-relaxed">
            一番賞為固定賞項商品，各賞別數量固定，抽取後不會重複。賞項依剩餘數量隨機決定，無法指定特定賞別。每次抽取結果一經確認即完成交易，不適用退款或換款。如對商品本身有疑慮，請於購買前詳閱商品說明。
          </p>
