...
const steps = [
  ...
  },
  {
    ...
    items: [
      {
        label: '前往我的倉庫',
        desc: '抽到的卡牌自動存入「個人中心 → 我的倉庫」，可申請寄送到家，也可以回收換回 G 幣。倉庫提供 30 天免費寄存，逾期系統將自動回收並返還相應代幣。',
      },
    ],
  },
];
...
export default function ChallengeRulesPage() {
  ...
  const rules = [
    ...
    {
      label: '倉庫寄存',
      desc: '卡牌存入我的倉庫後提供 30 天免費寄存。第 31 天起，系統將自動回收未申請配送的品項並返還相應代幣至帳戶，請務必在期限內申請配送。',
    },
    {
      label: '回收規則',
      desc: '我的倉庫內卡牌可隨時手動申請回收，系統將依卡牌回收價值返還相應代幣至帳戶。回收操作確認後無法還原，請確認後再執行。',
    },
    {
      label: '售後服務',
