...
const steps = [
  {
    ...
    items: [
      {
        label: '立即抽卡',
        desc: '點擊「立即抽卡」，使用 G幣 確認購買並抽取。每次抽取結果即時產生，無法取消或重抽。',
      },
    ],
  },
  {
    ...
    items: [
      {
        label: '查看結果',
        desc: '抽卡結果即時顯示，卡片自動存入帳號「我的倉庫」。可前往「個人中心 → 抽獎紀錄」查看歷史記錄。卡片稀有度由系統隨機決定，每次抽取均為獨立隨機。',
      },
      {
        label: '翻牌數字',
        desc: '翻開卡片時畫面上方跳出的數字（例如 +10,000.00）是依第三方卡牌市場行情以當日匯率換算成新台幣的參考值，僅供體感與參考，不是本平台的回收價，也不代表任何實際價值；實際回收價以「我的倉庫」顯示為準。行情每日更新，無行情資料的卡片不會顯示。',
      },
    ],
  },
  {
    ...
    items: [
      {
        label: '前往我的倉庫',
        desc: '進入「個人中心 → 我的倉庫」選取卡片後申請配送。倉庫提供 30 天免費寄存，逾期系統將自動回收品項並返還相應代幣，請務必在期限內申請配送。',
      },
    ],
  },
];
...
export default function CardRulesPage() {
  ...
  const rules = [
    ...
    {
      label: '倉庫寄存',
      desc: '卡片存入我的倉庫後提供 30 天免費寄存。第 31 天起，系統將自動回收未申請配送的品項並返還相應代幣至帳戶，請務必在期限內申請配送。',
    },
    {
      label: '免運條件',
    ...
    {
      label: '回收規則',
      desc: '我的倉庫內品項可隨時手動申請回收，系統將依商品類別計算回收金額，返還相應代幣至帳戶。回收操作確認後無法還原，請確認後再執行。',
    },
    {
      label: '售後服務',
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 pb-16 safe-header-offset">
      <SimplePageHeader title="抽卡規則" onBack={() => router.back()} maxWidth="max-w-[960px]" />
      ...
      <div className="max-w-[560px] md:max-w-[960px] mx-auto px-4 py-6 space-y-4 md:space-y-8">
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-100 dark:border-neutral-800 px-5 py-5">
          ...
          <p className="text-[13px] text-neutral-500 dark:text-neutral-400 leading-relaxed">
            抽卡商品為機率性商品，卡片稀有度及款式由系統隨機決定。每次抽取結果一經確認即完成交易，不適用無條件退款或換款。如對商品本身有疑慮，請於購買前詳閱商品說明。
          </p>
