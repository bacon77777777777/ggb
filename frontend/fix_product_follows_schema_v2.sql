...
-- Also fixed reference to users table instead of profiles, as profiles table appears to be missing in the current env.
...
CREATE TABLE public.product_follows (
  ...
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE, -- Changed to public.users
