...
三個老闆指定的細節：
- **類別膠囊固定寬 46px**：兩字（抽卡）與三字（自製賞）等寬，後面的商品名才對得齊
- **每列的卡片外框移除**，改用淡分隔線（`divide-y divide-neutral-100`）——
  白板本身就是白的，再套一層淺灰卡片只是把版面切得更碎
- 縮圖的白底也一併拿掉：外框上緣是粉紅漸層，白色方塊會浮出來

一次秀 **6 件**。內容區 66.5% × 卡片高 490px ≈ 326px，一列 52px、五條分隔線約 317px，
剛好塞滿不溢出。數量一律先量再定 —— 2×2 版與五列版都因為多塞一列而被切在白板下緣，
那看起來像壞掉、不像可以往下捲。
