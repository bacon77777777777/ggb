...
## v2026.08.06h｜2026-08-06｜🔴 一番賞／抽卡／自製賞加不了品項

### 症狀
商品編輯頁完全沒有「新增品項」按鈕，品項面板停在「剩餘 0 / 總計 0」。
PROD 534 號（ichiban／待上架）就是這樣，批量上架只匯了商品資料、品項補不進去。

### 根因：鎖定條件寫成看「類型」而不是看「封存了沒」
「新增品項」「點擊新增品項」「刪除品項」三處都擋在 `!isVerifiable`，而

    const isVerifiable = ['ichiban','card','custom'].includes(formData.type)

原意是**封存後**不能改品項（承諾值公布後動內容，「開賣前排定」就是假的），
但寫成看類型的結果是：**這三種類型只要建立時沒帶品項，之後永遠加不進去**。
534 號查下去是 `sealed_at = NULL`、品項 0 筆 —— 根本還沒封存就被鎖死。

### 修法
`isVerifiable` 改為判斷 `sealed_at` 是否非 NULL；未封存者比照一般商品可編輯。
API 本來就回傳 `sealed_at`，且不在廠商遮蔽欄位（那只擋 `seed` / `profit_rate`），不需改 API。

### 驗證（STG 交易內實測，DB 仍是最後防線）
- 對已封存的 76 號新增品項 → `guard_sealed_product` 擋下 `PRODUCT_SEALED`
- 對未封存商品新增品項 → 成功，且未被誤觸發自動封存（`is_active=false` 時 trigger 不跑）

> 前端判斷只是體驗，真正的保證在 DB。這次確認即使前端寫錯，已封存商品仍改不動。
