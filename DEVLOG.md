...
## v2026.07.19e｜2026-07-19｜盒玩販賣機傾倒動畫無縫銜接修正

### 修正：前排盒子傾倒動畫斷點問題（mode2 & mode3）
- **根因**：framer-motion `rotateX`（3D X 軸）vs 物理引擎 `rotate(rad)`（2D Z 軸）—— 不同旋轉軸，切換瞬間視覺跳回正面
- **解法**：改用純 CSS `@keyframes rotate`（2D Z 軸），與物理引擎完全同軸同 pivot（`50% 50%`），切換瞬間零跳動
- 傾倒角度調整為 -32°（前排盒子 `transformOrigin: 50% 50%`，後排保留 `bottom center`）
- 物理盒子初始角度對齊 CSS 結束狀態（`rand(-0.60, -0.50)` ≈ -30°~-34°），確保一氣呵成
