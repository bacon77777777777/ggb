...
外框與圓角比照 `GachaResultModal` 的中獎清單。

一次秀 **5 件**（原本 4 件）。白板高度是外框的 66%，在 max-w-330 的卡片上約 324px，
一列 48px＋間距 6px，五列剛好塞滿；試過六列，第六列會被切在白板下緣 ——
那看起來像壞掉、不像可以往下捲（同一個坑在 2×2 版型也踩過一次，這次先量再定數量）。
