...
## v2026.08.10c｜2026-08-10｜中獎音效改用盒玩取物那顆，取物當下不再出聲

盒玩立體販賣機（mode5）點「點擊取物」會響一次合成音效，緊接著恭喜獲得彈窗
又播 `getpopup.mp3`，同一個動作聽到兩個音效。老闆定案：**留取物那顆、
恭喜獲得彈窗改播它，取物本身不出聲**。

- mode5 的 `collectSfx`（粉紅噪音掃頻＋C 大調上行琶音）抽到 `lib/sfx.ts`
  成為 `playWinChime()`，全站彈窗共用；合成音效零載入，沒有首播延遲
- `GachaResultModal` 改用 `playWinChime()`，不再建 `Audio(getpopup.mp3)`
- `BlindboxMachineMode5.collect()` 拿掉發聲，其餘機械音（撞擊、齒輪、
  伺服、CTA 浮現的 ding）全部保留
- 一番賞 A賞／最後賞彈窗的 `getpopup.mp3` **維持原樣** —— 老闆確認
  「任何恭喜彈窗都該有音效」，那兩個不是這次要處理的重複

> 走錯一次：先前誤判成「A賞彈窗那份是舊的」而移除，等於把最後賞的恭喜
> 音效一起關掉，已 revert。要動音效前先確認是哪個機台、哪個動作。
