...
export default function AdminLayout({ children, pageTitle, pageSubtitle, breadcrumbs }: AdminLayoutProps) {
    ...
    () => [
      ...
      },
      {
        ...
        items: [
          ...
          { name: '小物管理', path: '/small-items', icon: IconPuzzle },
          { name: '回收商品管理', path: '/dismantled', icon: IconScissors },
      ...
      },
      {
        ...
        items: [
          ...
          { name: '廠商設定', path: '/suppliers/settings', icon: IconSliders },
        ],
      },
      {
        id: 'members',
  ...
  const PAGE_INFO: Record<string, string> = {
    ...
    '/settings/recycle': '設定五類商品的回收比例，基準為商品單抽價 —— 品項編輯頁不再有回收價欄位。轉蛋與盒玩各一個比例，一番賞／抽卡／自製賞另分大賞與一般賞（大賞由系統依品項總數自動判定）。回收在廠商結算怎麼拆帳（結算方式、差額分潤、廠商個別設定）在「廠商管理 → 廠商設定」。',
    '/suppliers/settings': '廠商的回收結算方式與差額分潤設定，含完整變更紀錄（誰、何時、從多少改成多少）。',
    '/small-items': '機台獎池用的小物品項庫，不上架、不售價，供機台出獎與直衝定價使用。',
    '/dismantled': '玩家回收的品項：逐筆明細與品項庫存盤點，可標記已再利用／已報廢並匯出給廠商。',
