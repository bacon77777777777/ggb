-- 606: 宅配／超商收件資料格式驗證（老闆 2026-08-24）
--
-- create_delivery_order 原本對收件人姓名／電話／地址完全不驗，亂填也收單。
-- 規則（與前台一致）：姓名 2–10 字、電話 09 開頭 10 碼、宅配地址 8–60 字且含縣市。
-- 函數本體取自 STG 現行版（兩環境 md5 相同），只插入開頭的驗證段。
