-- 396: 預設玩法 v2（回應：保底太高沒人轉 / 直擊不可高於最高品項 / 黃金序章固定 2 成）
-- 保底 200、觸發 0.2%、延續 30%（衰減 50%，僅自然觸發）、返還期望 38.7%（黃金序章 0.2×）
-- 直擊：價格 = 該檔次獎池最高品項價值（進位至檔次倍數），保證 1 件、不觸發延續
-- RTP ≈ 82%（毛利 ~18%），直擊毛利 ~16%

BEGIN;

ALTER TABLE public.slot_machines
  ADD COLUMN IF NOT EXISTS rush_from_direct BOOLEAN NOT NULL DEFAULT FALSE;

-- 1. 主題與機台參數
UPDATE public.slot_themes SET
  floor_spin_count = 200,
  trigger_rate = 0.002,
  continue_rate = 0.30,
...
UPDATE public.slot_machines m SET
  ...
  floor_counter = LEAST(m.floor_counter, 199),
...
-- 3. enter_slot_rush_direct：價格 = 該檔次最高品項價值進位；標記 rush_from_direct
...
BEGIN
  ...
  -- 定價：該檔次 RUSH 獎池最高品項價值，無條件進位至檔次金額倍數
  ...
  v_cost := CEIL(v_max_val / p_bet::NUMERIC) * p_bet;
  ...
  VALUES (
    v_user_id,
    -v_cost,
    FORMAT('直撃進入 RUSH：機台 #%s，檔次 %sG，池內最高 %sG', p_machine_id, p_bet, v_max_val),
    'system_direct_rush'
  );
...
-- 4. play_slot_locked：直擊 RUSH 不觸發延續（rush_from_direct）
