...
-- Add points_used column to draw_records to track G-equivalent cost when draw is paid with points
-- This enables per-supplier points deduction calculation in settlement reports

ALTER TABLE public.draw_records
  ADD COLUMN IF NOT EXISTS points_used INTEGER NOT NULL DEFAULT 0;

-- Rebuild play_gacha to write points_used per draw record
-- (Incorporates all fixes from migrations 237: expiry_date IS NULL, c.is_active check)
