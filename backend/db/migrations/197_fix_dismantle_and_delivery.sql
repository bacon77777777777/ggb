-- 197_fix_dismantle_and_delivery.sql
-- 修復 dismantle_prizes 使用正確的 admin_recycle_pool 欄位名稱
-- 修復 create_delivery_order 使用 tokens 而非 points

BEGIN;

-- ── 1. dismantle_prizes ───────────────────────────────────────────────────────
-- admin_recycle_pool 實際欄位：draw_record_id, user_id, product_id, prize_name, prize_level, recycle_value
...
BEGIN
  FOR v_record IN
    SELECT dr.id,
           pp.recycle_value, pp.total, pp.product_id, pp.name AS prize_name, pp.level AS prize_level,
           p.price
...
-- ── 2. create_delivery_order ──────────────────────────────────────────────────
-- users 表使用 tokens，不是 points
