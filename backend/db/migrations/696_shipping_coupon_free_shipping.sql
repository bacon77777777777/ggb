-- 696: 運費優惠券新增「免運」折扣類型，百分比運費券真的照百分比算（老闆 2026-09-05）
--
-- 後台新增折價券的折扣類型只有「固定金額／百分比」，老闆要一個「免運」。
-- 順手修一個沒人踩到過的洞：create_delivery_order 對運費券一律拿 discount_value 當 G 扣，
-- 百分比券（例如 10）會被當成折 10 G，不是折 10%。
--
-- ・coupons.discount_type 多一個 'free_shipping'（只給 scope='shipping' 用，後台表單也只在運費券才顯示）
-- ・create_delivery_order 的折抵改成依 discount_type：
--     free_shipping → 整筆運費｜percentage → ROUND(運費 × N%)｜fixed → LEAST(運費, N)
--   前台 frontend/lib/shippingCoupon.ts 的 calcShippingDiscount 同一套公式（FEE_MISMATCH 比的是折後淨額）
-- ・函式簽章不變，直接 CREATE OR REPLACE。函式本體取自 STG 現行版本（與 PROD 相同），只動折抵那兩處。
...
BEGIN
  ...
  IF p_coupon_id IS NOT NULL THEN
    SELECT uc.id AS uc_id, c.discount_type, c.discount_value INTO v_coupon
    ...
    END IF;
    -- 折抵依券的折扣類型算（migration 696）：免運＝整筆運費；百分比＝運費 × N% 四捨五入；
    -- 固定金額＝折抵 G 上限。前台 lib/shippingCoupon.ts 的 calcShippingDiscount 必須一字不差，
    -- 不然 FEE_MISMATCH 會擋下每一張帶券的單
    v_discount := CASE v_coupon.discount_type
      WHEN 'free_shipping' THEN v_fee
      WHEN 'percentage'    THEN LEAST(v_fee, ROUND(v_fee * COALESCE(v_coupon.discount_value, 0) / 100.0)::INTEGER)
      ELSE LEAST(v_fee, COALESCE(v_coupon.discount_value, 0)::INTEGER)
    END;
  END IF;
