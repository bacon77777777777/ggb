-- Update play_ichiban to support coupons
-- Also update play_gacha to pass coupon_id

-- 1. Update play_ichiban signature and logic
...
BEGIN
  ...
  -- Coupon Logic (Only for Token payment)
  ...
  IF p_use_points THEN
    v_total_cost_points := (v_product_price * v_count) * 4; -- Points calculation based on original price
  ...
  FOREACH v_ticket_no IN ARRAY p_ticket_numbers LOOP
    ...
    -- Insert notification for draw result
  ...
  IF v_product_remaining = 0 THEN
     ...
     IF v_last_one_prize IS NOT NULL THEN
       ...
       -- Insert notification for Last One
       INSERT INTO notifications (
          user_id,
          type,
          title,
...
-- 2. Update play_gacha signature and pass coupon_id
...
BEGIN
  ...
  -- Select available ticket numbers randomly
  SELECT ARRAY(
    SELECT t.num
  ...
  -- Delegate to play_ichiban with points flag and coupon_id
  RETURN public.play_ichiban(p_product_id, v_ticket_numbers, p_use_points, p_coupon_id);
END;
$$;
