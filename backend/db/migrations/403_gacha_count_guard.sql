-- 403: 轉蛋抽獎次數下限防護（修負數印幣漏洞）
--
-- 漏洞：play_gacha 的 p_count 無下限檢查。傳入負數時：
--   v_total_cost = price * 負數 = 負數
--   UPDATE users SET tokens = tokens - 負數 → 餘額「增加」，WHERE tokens >= 負數 恆真
--   FOR i IN 1..負數 LOOP 不執行 → 不扣庫存、不抽獎
--   結果：任何登入用戶打 /api/gacha 傳 count=-N 即可無限印幣。
-- 前端 API 只擋 !count（0/undefined），負數是 truthy 會直接放行，故必須在 DB 層擋。
--
-- 一番賞（play_ichiban）以票號陣列長度為次數、且驗證範圍，天生無此問題；
-- 老虎機（play_slot_locked / enter_slot_rush_direct）的 bet 必須精確匹配既有檔次，亦無此問題。
