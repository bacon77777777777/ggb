-- 394: 機台共用庫存（product_prizes.remaining 為準，同主題 5 台共用）
-- 1. RUSH 抽選改 LOOP：庫存 >0 才可抽；條件式扣減（行鎖防重複出獎）；
--    撞庫存自動排除重抽，用戶無感。獎池列 remaining / slot_prizes 庫存廢棄不用。
-- 2. enter_slot_rush_direct 直衝定價僅計入尚有庫存的品項。
-- 3. 初始庫存：機台類別品項全部設 5，商品 totals 同步。
...
-- 初始庫存：機台類別品項全部設 5，商品 totals 同步
UPDATE public.product_prizes pp
SET total = 5, remaining = 5
FROM public.products p
WHERE pp.product_id = p.id AND p.type = 'slot';
