-- 574_sell_my_state.sql
--
-- 接線第二批（購買鏈）最後一塊：一支 RPC 把「我的」整頁需要的東西一次吐出來，
-- 而且**直接吐成前台引擎的形狀**（見 app/sell/proto/mall.ts 的 orders/sellOrders/myList/cart）。
--
-- 為什麼在 SQL 做形狀轉換而不是在 JS：
-- 引擎是原型逐字移植的，渲染邏輯不能動（ROADMAP）。形狀對映擺在 SQL，
-- JS 端就只剩「把回傳值指給那幾個陣列」，之後跟原型 diff 還是乾淨的。
--
-- 步驟對照：DB step 1..5（待付款/待確認收款/待出貨/待收貨/完成）
--           引擎 st  0..4，取消為 9。所以 st = step - 1。
