-- 337_fix_coupon_is_used_column.sql
-- user_coupons 實際欄位是 status('unused'/'used'/'expired') + used_at
-- 但 play_gacha 函數仍在使用不存在的 is_used 欄位，導致優惠券抽獎 500
-- 修正：is_used = FALSE → status = 'unused'，SET is_used = TRUE → SET status = 'used'
