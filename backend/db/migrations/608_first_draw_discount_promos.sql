-- 608: 折扣型促銷（老闆 2026-08-24：A=前N抽折扣 first_n、B=每人首抽折扣 first_per_user）
--
-- promotions.type 新增 'first_n'（config: {"n": 100, "off_pct": 20} ＝ 前 100 個付費抽打 8 折）
-- 與 'first_per_user'（config: {"off_pct": 50} ＝ 每人在此商品的付費首抽 5 折、限一次）。
-- 折扣在 play_ichiban／play_gacha 伺服器端計價時套用（前端只能看，不能少報）；
-- 用量與折抵金額記 promotion_redemptions（bonus_count=0 區分於買N送M）。
-- 函數本體取自現行版（兩環境 md5 相同），只插入 DECLARE 三個變數＋折扣段＋優惠券互斥條件。
...
-- 前台試算用：這一次購買會折多少（伺服器同一套邏輯的唯讀版；訪客 auth.uid() 為 null，
-- first_per_user 會回 0）。實際折扣仍以 play_* 內的計算為準。
CREATE OR REPLACE FUNCTION public.promo_discount_quote(p_product_id bigint, p_count integer)
RETURNS jsonb
...
DECLARE
  ...
  v_off numeric;
BEGIN
  IF p_count IS NULL OR p_count < 1 THEN RETURN jsonb_build_object('discount', 0); END IF;
  SELECT price INTO v_price FROM public.products WHERE id = p_product_id;
  IF v_price IS NULL THEN RETURN jsonb_build_object('discount', 0); END IF;
  SELECT gp.* INTO v_promo FROM public.get_product_promotion(p_product_id) gp
  WHERE gp.type IN ('first_n', 'first_per_user');
  IF NOT FOUND THEN RETURN jsonb_build_object('discount', 0); END IF;
  v_off := LEAST(GREATEST(COALESCE((v_promo.config->>'off_pct')::numeric, 0), 0), 90);
  IF v_promo.type = 'first_n' THEN
    SELECT GREATEST(0, COALESCE((v_promo.config->>'n')::int, 0) - COALESCE(SUM(draw_count), 0)) INTO v_k
    FROM public.promotion_redemptions WHERE promotion_id = v_promo.id AND product_id = p_product_id;
    v_k := LEAST(v_k, p_count);
  ELSIF v_uid IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM public.promotion_redemptions
                     WHERE promotion_id = v_promo.id AND product_id = p_product_id AND user_id = v_uid)
     AND NOT EXISTS (SELECT 1 FROM public.draw_records
                     WHERE product_id = p_product_id AND user_id = v_uid) THEN
    v_k := 1;
  END IF;
  RETURN jsonb_build_object(
    'discount', floor(v_price * v_off / 100.0)::int * v_k,
    'discounted_count', v_k,
    'off_pct', v_off,
    'type', v_promo.type,
...
-- promotions.type 檢查約束放寬為三種型別
ALTER TABLE public.promotions DROP CONSTRAINT IF EXISTS promotions_type_check;
ALTER TABLE public.promotions ADD CONSTRAINT promotions_type_check CHECK (type IN ('bundle','first_n','first_per_user'));
