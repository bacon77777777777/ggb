-- 398: 佔用期限錨定「最後動作 +90 秒」
-- spin（play_slot_locked）與直擊（enter_slot_rush_direct）刷新 occupant/期限；
-- occupy/heartbeat/vacate 不再延長期限（前端 API 同步修改），重複進出無法卡台。
...
BEGIN
  ...
  UPDATE public.slot_machines SET
    ...
    occupancy_expires_at  = now() + interval '90 seconds',
