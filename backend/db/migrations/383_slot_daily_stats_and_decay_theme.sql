-- 383_slot_daily_stats_and_decay_theme.sql
-- 1. slot_themes 加 continue_rate_decay（後台可調，同步至旗下機台）
-- 2. slot_machines 加每日統計欄位（day_spins / day_rush / day_reset_date）
--    由 play_slot_locked 順帶維護：每次 spin 檢查台灣日期，跨日自動清零，不需 cron
-- 3. play_slot_locked / enter_slot_rush_direct 回傳 session 加入 day_spins / day_rush
...
ALTER TABLE public.slot_machines
  ADD COLUMN IF NOT EXISTS day_spins      INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS day_rush       INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS day_reset_date DATE;

-- ── 2. 重寫 play_slot_locked（382 + 每日統計）────────────────────────────────
...
-- ── 3. enter_slot_rush_direct：直撃計入 day_rush ─────────────────────────────
