  ...
  FOREACH v_ticket_no IN ARRAY p_ticket_numbers LOOP
    ...
    IF v_normal_qty = 0 THEN
        ...
        IF v_last_one_prize IS NOT NULL THEN
            ...
            INSERT INTO draw_records (
                user_id, product_id, product_prize_id, ticket_number, prize_level, status,
                txid_seed, txid_nonce, txid_hash, random_value, profit_rate, prize_name
            )
            VALUES (
                v_user_id, p_product_id, v_last_one_prize.id, 0, v_last_one_prize.level, 'in_warehouse',
                v_seed, v_nonce, v_hash, v_random, 1.0, v_last_one_prize.name
            );
