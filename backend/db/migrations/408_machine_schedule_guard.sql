-- 408: 機台檔期的伺服器端防護
--
-- 前台已依檔期蓋遮罩並禁用入場，但畫面擋不住直接打 API。
-- 與先前修補的佔用軟鎖同性質，故在 DB 函數層一併擋下：
--   未開始 → '機台尚未開放'
--   已結束 → '機台已結束'
-- 檔期繼承規則與前台一致：機台自身優先，留空則跟隨主題。
...
BEGIN
  ...
  -- 佔用硬鎖：他人佔用未過期的機台不可操作（過期即釋出，可合法接手含 RUSH 狀態）
  IF v_machine.occupant_id IS NOT NULL
     AND v_machine.occupant_id <> v_user_id
     AND v_machine.occupancy_expires_at IS NOT NULL
     AND v_machine.occupancy_expires_at > now() THEN
    RAISE EXCEPTION '機台使用中';
  END IF;

  -- 檔期防護：機台自身優先，留空則跟隨主題（與前台同一套繼承規則）
  SELECT COALESCE(v_machine.start_at, t.start_at), COALESCE(v_machine.end_at, t.end_at)
  INTO v_sched_start, v_sched_end
  FROM public.slot_themes t WHERE t.id = v_machine.theme_id;
  IF NOT FOUND THEN
    v_sched_start := v_machine.start_at;
    v_sched_end   := v_machine.end_at;
  END IF;

  IF v_sched_start IS NOT NULL AND now() < v_sched_start THEN
    RAISE EXCEPTION '機台尚未開放';
  END IF;
  IF v_sched_end IS NOT NULL AND now() > v_sched_end THEN
    RAISE EXCEPTION '機台已結束';
  END IF;
