-- 681: 配送彈窗商城化——運費優惠券＋配送備註
--
-- 老闆 2026-09-02：倉庫配送彈窗照商城結帳重做，含運費券與備註。
-- ・coupons.scope：'draw'（現有券全歸這類）｜'shipping' 運費券（discount_value＝折抵 G 上限）
-- ・orders.note／coupon_id／shipping_discount：備註、用了哪張券、折了多少
-- ・create_delivery_order 加 p_note、p_coupon_id（帶預設，舊呼叫不破）：
--   伺服器端驗券（本人、未用、未過期、shipping、啟用中）→ 折抵 → 原子核銷；
--   FEE_MISMATCH 比對的是折後淨額（前端顯示多少就付多少）。

ALTER TABLE public.coupons ADD COLUMN IF NOT EXISTS scope text NOT NULL DEFAULT 'draw';
ALTER TABLE public.coupons DROP CONSTRAINT IF EXISTS coupons_scope_check;
ALTER TABLE public.coupons ADD CONSTRAINT coupons_scope_check CHECK (scope IN ('draw', 'shipping'));

ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS note text;
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS coupon_id bigint;
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS shipping_discount integer NOT NULL DEFAULT 0;

-- 換簽名要先拔舊的，避免留一個同名不同參數的孤兒 overload
DROP FUNCTION IF EXISTS public.create_delivery_order(uuid, text, text, text, text, text, text, text, bigint[], integer);

CREATE OR REPLACE FUNCTION public.create_delivery_order(p_user_id uuid, p_recipient_name text, p_recipient_phone text, p_address text, p_logistics_type text, p_logistics_subtype text, p_store_id text, p_store_name text, p_draw_record_ids bigint[], p_delivery_fee_points integer, p_note text DEFAULT NULL, p_coupon_id bigint DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
