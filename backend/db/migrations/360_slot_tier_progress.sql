-- Migration 360: Per-tier floor progress for slot machines
-- Replaces single spins_since_rush with per-tier progress tracking
-- This prevents the exploit: "cheap spins to build floor, switch to expensive tier to trigger RUSH"

-- 1. Add tier_progress to slot_sessions
ALTER TABLE slot_sessions
  ADD COLUMN IF NOT EXISTS tier_progress JSONB NOT NULL DEFAULT '{}'::JSONB;

-- 2. Add event_slug to slot_machines (for banner → event page linking)
ALTER TABLE slot_machines
  ADD COLUMN IF NOT EXISTS event_slug TEXT;

-- 3. Migrate existing spins_since_rush into tier_progress for each session
-- (Best effort: attribute existing progress to the machine's first bet tier)
UPDATE slot_sessions ss
SET tier_progress = jsonb_build_object(
  (
    SELECT t->>'coins'
    FROM jsonb_array_elements(sm.bet_tiers) t
    ORDER BY (t->>'coins')::int ASC
    LIMIT 1
  ),
  ss.spins_since_rush
)
FROM slot_machines sm
WHERE ss.machine_id = sm.id
  AND ss.spins_since_rush > 0;

-- 4. Replace play_slot_locked with per-tier progress version
CREATE OR REPLACE FUNCTION play_slot_locked(p_machine_id bigint, p_bet bigint)
...
BEGIN
  ...
  -- Per-tier floor progress: only this tier's count matters for floor trigger
  ...
  -- ── Prize selection (unchanged logic) ────────────────────────────────────
  ...
  -- Update tier_progress (per-tier floor counter)
  IF v_rush_triggered THEN
    -- RUSH triggered: reset THIS tier's counter to 0
  ...
  ELSIF NOT v_in_rush THEN
    -- Normal spin: increment this tier's counter
    v_new_tier_progress := jsonb_set(
      COALESCE(v_session.tier_progress, '{}'::jsonb),
      ARRAY[v_actual_bet::text],
  ...
  ELSE
    -- During RUSH (no trigger): tier_progress unchanged
