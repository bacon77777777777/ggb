-- 612: 配送訂單記錄供貨廠商
--
-- 背景：orders.supplier_id 這個欄位早就在（FK → suppliers），但 create_delivery_order
-- 的 INSERT 從來沒寫過它，所以每一筆訂單都是 NULL。前台「配送管理」要顯示配送廠商
-- 就沒得讀（老闆 2026-08-24）。
--
-- 順便補上單一廠商的驗證：倉庫頁的公告列本來就寫著「訂單以廠商為單位分批出貨，
-- 每次申請限同一廠商品項」，但 RPC 完全沒擋 —— 前台的「全選」會把廠商鎖清掉，
-- 一路點下去就會做出一張違反自家規則的混廠商訂單。規則寫在畫面上就該在 DB 擋。

-- STG 的 orders 沒有 supplier_id（兩環境早就有欄位差，見下方 DEVLOG 備註），
-- 這裡補齊才跑得起來。PROD 已經有，IF NOT EXISTS 讓兩邊都能重複執行。
...
-- 既有訂單回填：從訂單裡的獎品往回查商品的廠商，只有整張訂單同一家才寫
UPDATE orders o
SET supplier_id = src.supplier_id
FROM (
  SELECT dr.order_id, MIN(p.supplier_id) AS supplier_id
  FROM draw_records dr
  JOIN products p ON p.id = dr.product_id
