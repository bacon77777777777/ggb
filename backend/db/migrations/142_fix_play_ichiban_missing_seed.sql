-- Fix play_ichiban to handle missing seed in products (e.g. from batch creation)
-- If seed is missing, generate one on the fly and save it.
...
BEGIN
  ...
  -- FIX: Handle missing seed (e.g. from batch creation)
  IF v_seed IS NULL THEN
    v_seed := md5(random()::text || clock_timestamp()::text);
    -- Save the generated seed back to the product for consistency
  ...
  FOREACH v_ticket_no IN ARRAY p_ticket_numbers LOOP
    ...
    -- seed is now guaranteed to be NOT NULL
    ...
    -- FILTER: Only notify if prize is in major_prizes (Big Prize)
  ...
  IF v_product_remaining = 0 THEN
     ...
     IF v_last_one_prize IS NOT NULL THEN
       ...
       -- Insert notification for Last One (Always notify)
       INSERT INTO notifications (
          user_id,
          type,
          title,
