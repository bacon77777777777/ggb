-- play_gacha 接受 status = 'active'（原本只接受 'selling'，導致 Product is not available 報錯）
-- 商品實際 status 值為 'active'，兩者對齊
...
BEGIN
  ...
  IF v_product.status NOT IN ('selling', 'active') THEN RAISE EXCEPTION 'Product is not available'; END IF;
