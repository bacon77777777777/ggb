...
-- Fix two bugs in play_gacha coupon validation:
-- 1. expiry_date > now() rejects NULL (永久有效) coupons — use (IS NULL OR > now())
-- 2. c.is_active not checked — deactivated coupons could still be applied
