 ...
 * 會產生跟正式流程不同的視覺結果。
 *
 * 作法：不塗抹、不裁切，保留原圖，只在浮水印所在的角落壓上白色圓角墊 + GGB logo。
 * 角落由 dengekiWm 的四角模板比對決定。全程本地運算，不呼叫付費服務。
 */
...
export async function brandCoverImage(buf: Buffer, corner: WmCorner): Promise<Buffer | null> {
  try {
