...
// ─── Intent routing ────────────────────────────────────────────────
// Fast-path intents: only A_revenue (deterministic calc) and J_execute
// (write ops needing confirmation). Everything else → handleOpenQuestion
// (Claude tool loop), which is accurate for any query type.
...
function matchIntentRegex(t: string): IntentType {
  ...
  if (/^(今天|昨天|這週|本月|近\d+天)?(營收|儲值金額|收入)/.test(t)) return 'A_revenue'
  return null
}

// Classifier only needs to distinguish A, J, and unknown.
// All other queries (庫存、訂單、退款、月結、用戶、分析...) go to tool loop.
const INTENT_CLASSIFIER_SYSTEM = `你是吉吉比後台 LINE 助理的意圖分類器。
根據訊息內容，回傳以下其中一個代碼。只回一個字母或 "unknown"，不要任何解釋。

A = 「昨天/今天/本週/本月/近7天」等明確時間 + 「營收/儲值/消費/抽獎」等財務數字查詢
J = 執行寫入操作（下架、補幣、凍結帳號、取消訂單、建折扣碼、填追蹤號、核准退款等）
unknown = 以上都不是（庫存、訂單、退款、月結、用戶查詢、流量分析等一律回 unknown）`
...
async function classifyIntent(text: string): Promise<IntentType> {
  ...
  // confirm/cancel: unambiguous single-word responses, no API call needed
...
- 問題與平台完全無關（天氣、新聞、一般知識、閒聊等）→ 只回一句「這個我幫不上忙，我是平台管理系統。」，絕不附帶任何平台數據或報表
...
**試試看**
- 轉蛋商品頁的免費體驗按鈕（GachaMachineVisual 組件）
- 點按後觸發轉蛋動畫，展示該商品目前最高等級獎品的外觀
- 完全不扣 G 幣，結果只是示範，不產生真實抽獎紀錄
- 商品已完抽時按鈕自動變灰（opacity-40）不可互動
- 觸擊 / 點擊 / 觸控 = 同一件事，都指玩家按下這個按鈕

**G 幣（代幣）**
- 玩家用法幣儲值換得的平台代幣，1 NT 對應固定匯率
- 抽獎時從玩家帳戶扣除；儲值成功後立即到帳
- 「G 幣」「代幣」「G」「G coin」指同一件事

**轉蛋 / 扭蛋（gacha）**
- 玩家在商品頁按「抽」消耗 G 幣，隨機得到獎品
- 每次抽完有動畫演出，獎品寄送到倉庫（warehouse）
- 商品頁 UI：轉蛋機圖片 → 選擇抽幾次 → 確認 → 動畫 → 結果彈窗

**倉庫**
- 玩家抽到的獎品統一放在倉庫，可申請出貨（寄到家）
- 一次可選多件合併出貨
...
每小時：
- 整點 新聞採集 Agent（news-agent）— 從 Google News RSS 抓最新一番賞/盒玩/轉蛋/卡牌情報，Claude 改寫成繁中，存入 news 表（預設下架草稿，管理員審閱後手動上架）

每週固定：
- 週一 11:00 市場情報官 — 自動爬 8 家【競品網站】，生成趨勢分析，存入 competitor_posts 表
- 週一 09:00 行銷長 — 加推競品動態 + 行銷週報

【重要】news 表 vs competitor_posts 表：
- 「文章/新聞/情報/最新文章」→ 查 news 表（新聞採集 Agent 寫的，是給玩家看的前台文章）
- 「競品/競品文章/對手動態」→ 查 competitor_posts 表（市場情報官爬的，是競品分析）
這兩個完全不同，不可混淆。
...
export async function askGbBro(question: string, lineUserId?: string): Promise<string> {
  ...
  switch (intent) {
    ...
    case 'J_execute':
      ...
      break
    default:
      // null and all other query types → full tool loop (accurate for any question)
