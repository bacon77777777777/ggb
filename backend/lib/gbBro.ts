...
function matchIntentRegex(t: string): IntentType {
  ...
  if (/庫存|剩幾個|還有幾|缺貨|快沒了|補貨|庫存量|低庫存/.test(t) && !/流量|瀏覽|人氣|熱門|點擊|排行/.test(t)) return 'C_inventory'
...
C = 庫存數量查詢（剩幾個、快賣完、缺貨、補貨、庫存夠不夠）— 注意：流量/瀏覽/熱門/人氣等分析類問題不屬於C
...
unknown = 以上都不是（包含：流量、瀏覽量、人氣、熱門排行、點擊率、轉化率、行為分析、用戶活躍度等分析類查詢）`
