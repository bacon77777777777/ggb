...
type ReportType = 'recharge' | 'consumption' | 'summary'
...
const TYPE_META: Record<ReportType, { title: string; breadcrumb: string }> = {
  recharge:    { title: '儲值明細', breadcrumb: '儲值明細' },
  consumption: { title: '消費明細', breadcrumb: '消費明細' },
  summary:     { title: '期間摘要', breadcrumb: '期間摘要' },
}
...
export default function ReportPage() {
  ...
  const reportType = (type as ReportType) || 'recharge'
  const meta = TYPE_META[reportType] ?? TYPE_META.recharge
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/admin/reports?tab=${reportType}&start=${start}&end=${end}`)
      ...
      else { setSummary(json.summary ?? null); setDailyBreakdown(json.dailyBreakdown ?? []) }
    } catch (e: any) {
  ...
  const handleExport = () => {
    if (reportType === 'recharge') {
      exportCSV(`儲值明細_${start}_${end}.csv`,
        ...
        rechargeData.map((r) => [
          formatDateTime(r.created_at), r.order_number ?? '',
          r.user?.name ?? '', r.user?.email ?? '',
          String(r.amount ?? 0), String(r.bonus ?? 0),
          r.payment_method ?? '', STATUS_LABEL[r.status] ?? r.status,
        ])
      )
      ...
      exportCSV(`消費明細_${start}_${end}.csv`,
        ...
        consumptionData.map((d) => [
          formatDateTime(d.created_at), d.user?.name ?? '', d.user?.email ?? '',
          d.product?.name ?? '', String(d.product?.price ?? 0),
          d.prize_level ?? '', d.prize_name ?? '', STATUS_LABEL[d.status] ?? d.status,
        ])
      )
    } else if (summary) {
      const rows: string[][] = [
      ...
      exportCSV(`期間摘要_${start}_${end}.csv`, ['項目', '數值'], rows)
    }
  }
  ...
  return (
    <AdminLayout
      ...
      breadcrumbs={[{ label: '報表' }, { label: meta.breadcrumb, href: `/reports/${reportType}` }]}
    >
                  ...
                  <tbody className="divide-y divide-neutral-100">
                    {consumptionData.map((d) => (
                      <tr key={d.id} className="hover:bg-neutral-50">
                        ...
                        <td className="px-4 py-2">
                          {d.prize_level ? (
                            <span className="px-2 py-0.5 bg-purple-100 text-purple-700 rounded-full text-xs font-medium">{d.prize_level}</span>
                          ) : '—'}
                        </td>
