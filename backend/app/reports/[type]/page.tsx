...
export default function ReportPage() {
  ...
  const typeColumns3: Column<any>[] = [
    ...
    },
    {
      ...
      label: "會員編號",
      className: "whitespace-nowrap",
      render: (d) => <MemberNo no={d.user?.member_no} uuid={d.user?.id} />,
    },
    {
      key: "c1",
      label: "暱稱",
  ...
  const handleExport = () => {
    if (reportType === 'recharge') {
      exportCSV(`儲值明細_${start}_${end}.csv`,
        ['日期', '訂單編號', '暱稱', '會員編號', '金額(TWD)', '贈點', '付款方式', '狀態'],
        rechargeData.map(r => [formatDateTime(r.created_at), r.order_number ?? '', r.user?.name ?? '', (r.user?.member_no ? String(r.user.member_no) : ''), String(r.amount ?? 0), String(r.bonus ?? 0), r.payment_method ?? '', STATUS_LABEL[r.status] ?? r.status])
      )
    } else if (reportType === 'consumption') {
      exportCSV(`消費明細_${start}_${end}.csv`,
        ['日期', '暱稱', '會員編號', '商品', '消耗代幣(G)', '獎品等級', '獎品名稱', '狀態'],
        consumptionData.map(d => [formatDateTime(d.created_at), d.user?.name ?? '', (d.user?.member_no ? String(d.user.member_no) : ''), d.product?.name ?? '', String(d.product?.price ?? 0), d.prize_level ?? '', d.prize_name ?? '', STATUS_LABEL[d.status] ?? d.status])
      )
