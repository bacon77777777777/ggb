...
import { AdminLayout, PageCard, SearchToolbar, SortableTableHeader, StatsCard, MemberNo } from '@/components'
...
/*
 * 這一頁是**已完成**的回收流水帳：玩家按下回收、代幣當下就退進他帳戶了，
 * 每一筆都是完成式。所以這裡不該出現任何「待處理」之類的狀態
 * （老闆 2026-08-31：「回收紀錄是用戶已回收且拿到退回代幣後的回收紀錄啊」）。
 *
 * 實體那件貨後來怎麼了（再利用／報廢）是另一件事，屬於「回收品項管理」
 * （/recycle-inventory）的實體盤點，不屬於這裡。原本這頁有一個「處置」欄
 * 與勾選批次標記，1,918 筆全掛「待處理」，其中 915 筆還是轉蛋／盒玩 ——
 * 那兩類回收後直接回原商品，根本不需要處置，數字等於灌了一半。已整組移除。
 *
 * DB 的 admin_recycle_pool.status（migration 617）與 PATCH /api/admin/dismantled
 * 都還在，之後要把標記功能做進「回收品項管理」時可以直接接。
 */
