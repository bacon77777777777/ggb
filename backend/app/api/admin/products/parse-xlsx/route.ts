...
function parseWithFieldMap(rows: any[][], headers: string[], fieldMap: FieldMap): ParsedProduct[] {
  ...
  return rows
    ...
    .map((row, i) => {
      ...
      // Image：外部 URL 完全忽略；自己 Storage URL → image_url；純檔名 → raw_image_name
      const imgRaw = imgIdx >= 0 ? str(row, imgIdx) || null : null
      const image_url      = (isHttpUrl(imgRaw) && isOurStorageUrl(imgRaw!)) ? imgRaw : null
      ...
      const prizes = prizeIdxs.map((pidx, vi) => {
        ...
        return {
          name: prizeName,
          qty: qty || undefined,
          image_url:      (isHttpUrl(imgRawP) && isOurStorageUrl(imgRawP!)) ? imgRawP : null,
