...
export async function GET(req: NextRequest, { params }: { params: Promise<{ prizeId: string }> }) {
  ...
  // Fetch extra to account for bot filtering (bots may take some slots)
  ...
  // Filter out bots and ghost accounts (not in users table), then slice to requested limit
  ...
  // Total non-bot count (approximate: subtract bot ratio from raw count)
  ...
  return NextResponse.json({ rows, total: rawCount ?? 0 })
}
